...
namespace dotnetCampus.OpenXmlUnitConverter
{
    ...
    public readonly struct MillisecondTime
    {
        ...
        public long ToTicks => IsIndefinite
            ? throw new InvalidOperationException($"The {nameof(MillisecondTime)} is indefinite. Can not convert to Tick.")
            : ToMillisecond * TicksPerMillisecond;

        /// <summary>
        /// 返回使用 <see cref="TimeSpan"/> 表示的时间
        /// </summary>
        public TimeSpan ToTimeSpan => IsIndefinite
            ? throw new InvalidOperationException($"The {nameof(MillisecondTime)} is indefinite. Can not convert to {nameof(TimeSpan)}.")
            : TimeSpan.FromMilliseconds(ToMillisecond);
