    ...
    public static partial class PackUriHelper
    {
        ...
        public static Uri GetRelationshipPartUri(Uri partUri)
        {
            ...
            if (IsRelationshipPartUri(partUri))
                throw new ArgumentException(partUri.ToString());
        ...
        public static Uri GetSourcePartUriFromRelationshipPartUri(Uri relationshipPartUri)
        {
            ...
            if (!IsRelationshipPartUri(relationshipPartUri))
                throw new ArgumentException(relationshipPartUri.ToString());
        ...
        private static Exception? GetExceptionIfPartUriInvalid(Uri partUri, out string partUriString)
        {
            ...
            if (partName.Length == 0)
                return new ArgumentException(partName);
        ...
        private static ArgumentException? GetExceptionIfAbsoluteUri(Uri uri)
        {
            if (uri.IsAbsoluteUri)
                return new ArgumentException(uri.ToString());
            else
                return null;
        }
                ...
                if (segments.Length > 3 && result == true)
                {
                    if ((segments[segments.Length - 1]).EndsWith(RelsrelsUpperCaseExtension, StringComparison.Ordinal))
                    {
                        ...
                        if (string.CompareOrdinal(segments[segments.Length - 3], RelationshipPartUpperCaseSegmentName) == 0)
                            throw new ArgumentException(RelationshipPartUpperCaseSegmentName);
                    }
                }
