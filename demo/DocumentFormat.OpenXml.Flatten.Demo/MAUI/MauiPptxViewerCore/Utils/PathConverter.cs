...
namespace MauiPptxViewerCore.Utils;

public static class PathConverter
{
    public static PathF ToPath(string pathString)
    {
        var pathF = new PathF();
            ...
            switch (cmd)
            {
                ...
                case 'S': // Smooth cublic Bezier
                    ...
                    do
                    {
                        ...
                        pathF.CurveTo(p, secondLastPoint, lastPoint);
                ...
                case 'T': // Smooth quadratic Bezier
                    ...
                    do
                    {
                        ...
                        pathF.QuadTo(secondLastPoint, lastPoint);
        ...
        void ThrowBadToken()
        {
            throw new FormatException(string.Format("UnexpectedToken \"{0}\" into {1}", currentPathString,
                currentIndex - 1));
        }
        ...
        double ReadNumber(bool allowComma)
        {
            ...
            else
            {
                ...
                {
                    throw new FormatException(string.Format("UnexpectedToken \"{0}\" into {1}", start,
                        currentPathString));
                }
            }
        }
        ...
        void SkipDigits(bool signAllowed)
        {
            // Allow for a sign
            if (signAllowed && More() &&
                (currentPathString[currentIndex] == '-' || currentPathString[currentIndex] == '+'))
            {
                currentIndex++;
            }
    ...
    //public static PathF[] ToPath(SvgPath svgPath)
    //{
    //    if (svgPath.MultiShapePaths != null)
    //    {
    //        var pathList = new PathF[svgPath.MultiShapePaths.Length];
    //        for (var i = 0; i < svgPath.MultiShapePaths.Length; i++)
    //        {
    //            svgPath.MultiShapePaths[i].
    //        }
    //    }
